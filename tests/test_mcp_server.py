...
def _make_mixed_index(tmp_path: Path) -> tuple[FileSystemStore, AgenticIndex]:
...
class TestGetDetailsUrl:
    ...
    async def test_fetches_url_and_returns_content(self, tmp_path, monkeypatch):
        from agentic_index.server import mcp_server
        ...
        with patch("agentic_index.server.mcp_server.httpx.AsyncClient", return_value=mock_client):
...
class TestGetDetailsLocalFile:
    ...
    async def test_reads_local_file_no_http(self, tmp_path):
        ...
        with patch("agentic_index.server.mcp_server.httpx.AsyncClient") as mock_http:
